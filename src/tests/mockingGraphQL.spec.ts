...
describe("Mocking graphQL requests", () => {
    it("It can mock multiple graphQL requests to the same URL but using the query name found in the request payload to have mock-request determine what to send when", async () => {
    ...
    it("It can mock multiple graphQL requests to the same URL but using the query name found in the query parameters to have mock-request determine what to send when", async () => {
        const firstMockedRequest = mockRequest({
            graphQLQueryName: "GetFirstThings",
