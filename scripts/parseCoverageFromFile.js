...
const readCoverageHTML = () => {
    const fileToRead = path.join(__dirname, "./../coverage/lcov-report/index.html")
    try {
...
const retrieveNumberStatistics = (match, returnUnknown) => {
    ...
    if (!numberMatches) {
        if (returnUnknown) {
            ...
            throw new Error(`Unable to retrieve the number matches from the HTML`)
        }
    } else {
...
const retrieveCoverageStatistics = (html, type, returnUnknown = false) => {
    ...
    if (!matches) {
        throw new Error(`Unable to parse coverage statistics in the HTML for type $"{type}".`)
    }
...
exports.parseCoverageFromFile = (options = {}) => {
    ...
    const statementData = retrieveCoverageStatistics(htmlFile, "statements", options.returnUnknown)
    const branchesData = retrieveCoverageStatistics(htmlFile, "branches", options.returnUnknown)
    const functionsData = retrieveCoverageStatistics(htmlFile, "functions", options.returnUnknown)
    const linesData = retrieveCoverageStatistics(htmlFile, "lines", options.returnUnknown)
    return {
        statementData,
        branchesData,
        functionsData,
